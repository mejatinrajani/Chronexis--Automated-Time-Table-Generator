...
def solve_minimal_timetable(data: MinimalData):
    ...
    # --- 3. Resource Balancing & Student Happiness (The Core Logic) ---
    obj_vars = []
    
    # A. Faculty Load Balancing
    for fac in data.faculty:
        total_fac_load = []
        for sec in data.sections:
            for sub in data.hours[sec].keys():
                if fac in data.competencies.get(sub, []):
                    hrs = data.hours[sec][sub]
                    load_v = model.NewIntVar(0, hrs, f"lv_{fac}_{sec}_{sub}")
                    model.Add(load_v == hrs).OnlyEnforceIf(fac_assign[(sec, sub, fac)])
                    total_fac_load.append(load_v)
        if total_fac_load:
            actual_load = model.NewIntVar(0, 40, f"act_load_{fac}")
            model.Add(actual_load == sum(total_fac_load))
            # Penalty if load exceeds 28 hours (prevents "Prof. Sharma Syndrome")
            is_overworked = model.NewBoolVar(f"over_{fac}")
            model.Add(actual_load > 28).OnlyEnforceIf(is_overworked)
            obj_vars.append(is_overworked * 10000)

    # B. Room Spread Preference
    for r in data.rooms:
        total_room_usage = sum(assign_room[(sec, sub, r, s_idx)] for sec in data.sections 
                               for sub in data.hours[sec].keys() for s_idx in range(data.num_slots) 
                               if (sec, sub, r, s_idx) in assign_room)
        obj_vars.append(total_room_usage * 10) # Tiny penalty to favor empty rooms

    # C. Squeezing Gaps & Loneliness (Student Happiness)
    for sec in data.sections:
        for s_idx in range(data.num_slots):
        ...
        for day_idx, indices in slots_by_day.items():
            ...
            # Rule: Minimum 3 classes per day
            is_under_3 = model.NewBoolVar(f"u3_{sec}_{day_idx}")
            model.Add(daily_sum < 3).OnlyEnforceIf(is_under_3)
            model.Add(is_under_3 == 0).OnlyEnforceIf(is_active.Not())
            obj_vars.append(is_under_3 * 15000) # Very heavy penalty

            for i, s_idx in enumerate(indices):
                # Penalty: Gaps (Students hate these the most)
                gap = model.NewBoolVar(f"gap_{sec}_{s_idx}")
                model.AddBoolAnd([is_active, is_occ[s_idx].Not()]).OnlyEnforceIf(gap)
                obj_vars.append(gap * 20000) # MASSIVE penalty for mid-day gaps

                # Penalty: Loneliness (Back-to-back logic)
                neigh = [indices[j] for j in [i-1, i+1] if 0 <= j < len(indices)]
                # Check for lunch break bridge
                real_neighbors = [n for n in neigh if data.slots[s_idx].start_time == data.slots[n].end_time or data.slots[n].start_time == data.slots[s_idx].end_time]
                
                if real_neighbors:
                    has_n = model.NewBoolVar(f"hasn_{sec}_{s_idx}")
                    model.AddMaxEquality(has_n, [is_occ[n] for n in real_neighbors])
                    lonely = model.NewBoolVar(f"lon_{sec}_{s_idx}")
                    model.AddBoolAnd([is_occ[s_idx], has_n.Not()]).OnlyEnforceIf(lonely)
                    obj_vars.append(lonely * 25000) # ABSOLUTE penalty for isolated classes

                # Early-Bird Preference: Fill morning slots first
                obj_vars.append(is_occ[s_idx] * (i * 50)) 

    # D. Credits & Labs (Hard)
    for sec in data.sections:
        for sub, credits in data.hours[sec].items():
            model.Add(sum(is_start[(sec, sub, s)] for s in range(data.num_slots)) == credits)
            if data.subject_durations.get(sub, 1) == 2:
