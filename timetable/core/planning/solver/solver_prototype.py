...
def solve_minimal_timetable(data: MinimalData):
    ...
    for sec in data.sections:
        ...
        for sub, credits in data.hours[sec].items():
            ...
            if duration == 2:
                # LAB RULE: Each class is 2 consecutive hours
                # Total slots occupied = Credits * 2
                model.Add(sum(assign[(sec, sub, s)] for s in range(data.num_slots)) == num_classes * 2)

                for s in range(data.num_slots):
                    # Check if session can start (not last slot of day)
                    if s < data.num_slots - 1 and data.slots[s].day_index == data.slots[s+1].day_index:
                        # Start at s implies s and s+1 are both occupied
                        model.Add(assign[(sec, sub, s)] == 1).OnlyEnforceIf(is_start[(sec, sub, s)])
                        model.Add(assign[(sec, sub, s+1)] == 1).OnlyEnforceIf(is_start[(sec, sub, s)])
                        
                        # Room Continuity: Stay in the same room for both hours
                        sub_type = data.subject_types[sub]
                        valid_rooms = [r for r in data.rooms if data.room_types[r] == sub_type]
                        for r in valid_rooms:
                            model.Add(assign_room[(sec, sub, r, s)] == assign_room[(sec, sub, r, s+1)]).OnlyEnforceIf(is_start[(sec, sub, s)])
                    else:
                        # Block cannot start if it would spill over to the next day
                        model.Add(is_start[(sec, sub, s)] == 0)
            else:
                # THEORY RULE: Each class is 1 hour
                # Total slots occupied = Credits * 1
                model.Add(sum(assign[(sec, sub, s)] for s in range(data.num_slots)) == num_classes)
                for s in range(data.num_slots):
                    model.Add(assign[(sec, sub, s)] == is_start[(sec, sub, s)])

            # C. Subject Distribution: Max 1 block per day
            for day_idx, day_slot_indices in slots_by_day.items():
                model.AddAtMostOne(is_start[(sec, sub, s_idx)] for s_idx in day_slot_indices)
